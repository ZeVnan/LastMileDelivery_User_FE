...
export const createNotification = async(order, about, status, token) => {
    ...
    try {
        const response = await fetch(`https://waseminarcnpm2.azurewebsites.net/protected/notification`, {
        method: 'POST',
        headers: {
        ...
        //console.log("Create Notification", `${response.status}`)
    }
    catch(error){
...
export const pushNotification = async(userId, title, message, token) => {
    try {
        ...
        //console.log("Push Notification", `${response.status}`);
    }
    catch (error){
